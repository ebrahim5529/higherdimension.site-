...
export function ContractPreviewModal({
  ...
  // حساب الإجمالي
  const equipmentTotal = rentalDetails.reduce((sum, item) => sum + item.total, 0);
  const totalAmount = equipmentTotal + transportCost - totalDiscount;
  ...
  const getDurationText = (duration: number, durationType: 'daily' | 'monthly') => {
    if (durationType === 'daily') {
      return `${duration} ${duration === 1 ? 'يوم' : 'يوم'}`;
    } else {
      return `${duration} ${duration === 1 ? 'شهر' : 'شهر'}`;
    }
  };
...
• القيمة الإجمالية: ${totalAmount.toFixed(2)} ر.ع

• تاريخ العقد: ${formatDate(contractDate)}
...
شركة البعد العالي للتجارة`;
  ...
  const contractDateAr = formatDate(contractDate);
  const contractDateEn = formatDateShort(contractDate);
  const [day, month, year] = contractDateEn.split('/');
        ...
        <DialogContent className="max-w-5xl max-h-[95vh] overflow-y-auto p-0 bg-white rounded-lg shadow-2xl w-full no-print">
          ...
          <div className="p-8 sm:p-12 bg-white contract-content" dir="rtl" style={{ fontFamily: 'Arial, sans-serif' }}>
            ...
            <div className="flex items-start justify-between mb-8 pb-4 border-b-2 border-black contract-header">
              ...
              <div className="text-left">
                <div className="bg-white border-2 border-black rounded-lg px-6 py-4">
                  ...
                  <p className="text-3xl font-bold text-black">{contractNumber}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 text-base leading-relaxed text-black contract-date">
              <p>
                إنه في يوم: <span className="font-semibold">{contractDateAr}</span> الموافق{' '}
                <span className="font-semibold border-b border-black px-2">{day}</span> /{' '}
                <span className="font-semibold border-b border-black px-2">{month}</span> /{' '}
                <span className="font-semibold border-b border-black px-2">{year}</span> م، تم الإتفاق بين كل من:
              </p>
            </div>
            ...
            <div className="mb-6 space-y-3 contract-sections">
              <div className="bg-white p-4 rounded-lg border-r-4 border-black">
                ...
                <p className="text-base text-black">
                  شركة البعد العالي للتجارة، س.ت: <span className="font-semibold">1208502</span>، ومقرها ولاية السيب، هاتف:{' '}
                  <span className="font-semibold">٩٣٠٩٩٩١٤</span>.
                </p>
              </div>
              <div className="bg-white p-4 rounded-lg border-r-4 border-black">
                <p className="text-lg font-bold mb-2 text-black">الطرف الثاني (المستأجر):</p>
                <p className="text-base mb-2 text-black">
                  شركة / مؤسسة: <span className="font-semibold">{customerName}</span>
                </p>
                <p className="text-base mb-2 text-black">
                  س.ت: <span className="font-semibold">{customerNumber}</span>
                </p>
                  ...
                  <tbody>
                    {rentalDetails.map((item, index) => (
                      <tr key={item.id} className="border-2 border-black">
                        ...
                        <td className="border-2 border-black p-3 text-center text-black">{item.itemDescription || 'تأجير'}</td>
                        <td className="border-2 border-black p-3 text-center font-semibold text-black">{item.quantity}</td>
                        <td className="border-2 border-black p-3 text-center text-black">{formatDateShort(item.startDate)}</td>
                        <td className="border-2 border-black p-3 text-center text-black">{formatDateShort(item.endDate)}</td>
                        <td className="border-2 border-black p-3 text-center font-semibold text-black">
                          {getDurationText(item.duration, item.durationType)}
                        </td>
            ...
            <div className="grid grid-cols-2 gap-8 mt-12 pt-8 border-t-2 border-black signatures">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-black mt-2">{customerName}</p>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-black text-center text-sm text-black contract-footer">
              <p className="mb-1">
                رقم العقد: <span className="font-semibold">{contractNumber}</span>
              </p>
              <p>
                تاريخ الإصدار: <span className="font-semibold">{contractDateAr}</span>
              </p>
            </div>
          </div>
        </DialogContent>
