...
export default function CreateDepartment({ managers }: CreateDepartmentProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    post('/employees/departments', {
      data: submitData,
      onSuccess: () => {
        showToast.success('تم الإنشاء بنجاح', 'تم إضافة القسم بنجاح');
        router.visit('/employees/departments');
      },
      onError: (errors) => {
        if (errors.message) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end gap-4 pt-6 border-t">
            ...
            <Button type="submit" disabled={processing} className="flex items-center gap-2 bg-[#913D95] hover:bg-[#7A2F7D]">
              ...
              {processing ? 'جاري الحفظ...' : 'حفظ القسم'}
            </Button>
          </div>
        </form>
      </div>
